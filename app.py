...
<h1>🚀 METAR 智能监控终端</h1>
<div style='font-size:12px;'>更新时间：{now_local().strftime('%Y-%m-%d %H:%M:%S')}</div>
</div>
""", unsafe_allow_html=True)
...
if "audio_unlocked" not in st.session_state:
    if st.button("🔊 启用声音提醒"):
        ...
        <script>
        var audio = document.getElementById("unlockAudio");
        if (audio) { audio.play(); }
        </script>
        """, unsafe_allow_html=True)
...
if is_new and len(data) >= 2:
    ...
    st.info(f"温度变化：{prev_temp} → {curr_temp}")

    if st.session_state.get("audio_unlocked"):
        st.markdown(f"""
        <audio id="alertAudio">
            <source src="https://www.soundjay.com/buttons/sounds/button-3.mp3?{datetime.utcnow().timestamp()}" type="audio/mpeg">
        </audio>
        <script>
